...
def retry_openai_chat_completion(client, model, messages, temperature,
    ...
    (no exponential backoff sleep).  Non-rate-limit errors still use the
    ...
    Args:
        ...
        current_key (str): The API key currently in use (needed to mark it
            as rate-limited in the pool).
    ...
    for attempt in range(max_retries):
        try:
            ...
            if isinstance(e, openai.RateLimitError) and key_pool is not None:
                ...
                print(f"[KeyPool] Rotated openai API key after rate limit (attempt {attempt+1}/{max_retries}).")
                continue
